...
void read_settings() {
    ...
    if (preferences.begin("settings", false) ) {                                
        ...
        settingsCache.OcppMeterManual = OcppMeterManual;
...
void write_settings(void) {
 ...
 if (preferences.begin("settings", false) ) {
    ...
    PREFS_PUT_UCHAR_IF_CHANGED("OcppMtrManual", OcppMeterManual, OcppMeterManual);
    // Strings are compared against NVS rather than a cache field: two 26-byte
    // mirrors would cost more RAM than the reads they save, and these change
    // once during onboarding.
    if (!preferences.isKey("OcppMtrType") || preferences.getString("OcppMtrType") != String(OcppMeterType))
        preferences.putString("OcppMtrType", OcppMeterType);
    if (!preferences.isKey("OcppMtrSerial") || preferences.getString("OcppMtrSerial") != String(OcppMeterSerial))
        preferences.putString("OcppMtrSerial", OcppMeterSerial);
