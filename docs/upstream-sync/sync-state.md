...
| 5 | `543af26` (+`7353d50`) | 2026-04-01 | stegen | EtherLCD support: Ethernet add-on board that replaces the LCD board (#349) | **Integration complete — pending on-device test** (fork issue #168) | P3 | branch `fix/issue-168-etherlcd` | Full runtime wiring now applied on top of the earlier cherry-pick: `setup()` CH390D probe/init, `getButtonState()` CH32V003 read, `network_common.cpp` `NetworkConnected()`/`onGotIP()`/`startNetworkServices()` refactor, Ethernet-priority `handleWIFImode()`, deferred `WIFImodeChanged`, OCPP lifecycle over `NetworkConnected()`, `eth.*` `/settings` JSON, plus panic fix `7353d50`. Decision logic extracted to pure-C `net_iface.c` with 9 native tests. Builds ESP32 (Flash 88.9%) + CH32, 55 native suites green. **Final gate: on-device bring-up on a real EtherLCD board** per the checklist in [`analysis-543af26-etherlcd.md`](analysis-543af26-etherlcd.md). |
